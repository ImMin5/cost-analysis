...
class CostManager(BaseManager):
    ...
    def create_cost(self, params, execute_rollback=True):
        ...
        if "usd_cost" not in params:
            ...
            if original_currency == "USD":
                ...
                if rate is None:
                    raise ERROR_UNSUPPORTED_CURRENCY(
                        supported_currency=list(self.exchange_rate_map.keys())
                    )
    ...
    def analyze_costs_with_cache(
        self, query, query_hash, domain_id, target="SECONDARY_PREFERRED"
    ):
        query["target"] = target
    ...
    @cache.cacheable(
        key="analyze-monthly-costs:{domain_id}:{query_hash}", expire=3600 * 24
    )
    ...
    def _create_cost_query_history_v2(self, query, granularity, start, end, domain_id):
        ...
        self.create_cost_query_history(
            domain_id, analyze_query, query_hash, granularity, start, end
        )
    ...
    def create_cost_query_history(
        self, domain_id, query, query_hash, granularity=None, start=None, end=None
    ):
    ...
    @cache.cacheable(
        key="stat-monthly-costs:{domain_id}:{query_hash}", expire=3600 * 24
    )
    ...
    def remove_stat_cache(domain_id):
        ...
        cache.delete_pattern(f"analyze-costs:{domain_id}:*")

    @staticmethod
    def is_monthly_cost(granularity, start, end):
        if (
            granularity in ["ACCUMULATED", "MONTHLY"]
            and start.day == 1
            and end.day == 1
        ):
            return True
    ...
    def add_date_range_filter(self, query, granularity, start: date, end: date):
        ...
        if self.is_monthly_cost(granularity, start, end):
            query["filter"].append(
                {"k": "billed_month", "v": start.strftime("%Y-%m"), "o": "gte"}
            )

            query["filter"].append(
                {"k": "billed_month", "v": end.strftime("%Y-%m"), "o": "lt"}
            )

        else:
            query["filter"].append(
                {"k": "billed_date", "v": start.strftime("%Y-%m-%d"), "o": "gte"}
            )

            query["filter"].append(
                {"k": "billed_date", "v": end.strftime("%Y-%m-%d"), "o": "lt"}
            )

        return query

    def make_accumulated_query(
        self,
        group_by,
        limit,
        query_filter,
        ...
        return {"aggregate": aggregate, "filter": query_filter}

    def make_trend_query(
        self,
        granularity,
        group_by,
        limit,
        ...
        aggregate = [
            {
                "group": {
                    "keys": self._get_keys_from_group_by(group_by)
                    + self._get_keys_from_granularity(granularity),
                    "fields": [
                        {"key": "usd_cost", "name": "usd_cost", "operator": "sum"}
                    ],
                }
            ...
            {"sort": {"key": "total_usd_cost", "desc": True}},
        ]
        ...
        aggregate.append(
            {
                "project": {
                    "fields": [
                        {"key": "total_usd_cost", "name": "total_usd_cost"},
                        {
                            "key": "usd_cost_values",
                            "name": "usd_cost",
        ...
        if include_usage_quantity:
            aggregate[-1]["project"]["fields"] += [
                {"key": "total_usage_quantity", "name": "total_usage_quantity"},
                {
    ...
    def sum_costs_by_project_group(
        self, response, granularity, group_by, domain_id, include_usage_quantity=False
    ):
        ...
        project_df = pd.DataFrame(
            projects_info, columns=["project_id", "project_group_id"]
        )
        ...
        if granularity == "ACCUMULATED":
            ...
            return {"results": join_df.to_dict("records")}

        else:
            aggr_values = {"usd_cost": list, "total_usd_cost": sum}
            ...
            return {"results": changed_results}

    @staticmethod
    def sum_costs_over_limit(
        response, granularity, limit, include_usage_quantity=False
    ):
        results = response.get("results", [])
        changed_results = results[:limit]

        if granularity == "ACCUMULATED":
            others = {"is_others": True, "usd_cost": 0}
            ...
            others = {"is_others": True, "total_usd_cost": 0, "usd_cost": {}}
    ...
    def _get_keys_from_granularity(granularity):
        ...
        if granularity == "DAILY":
            keys.append({"key": "billed_date", "name": "date"})
        elif granularity == "MONTHLY":
            keys.append({"key": "billed_month", "name": "date"})
        elif granularity == "YEARLY":
            keys.append({"key": "billed_year", "name": "date"})
            ...
            projects_info.append(
                {
                    ...
                    "project_group_id": project_info["project_group_info"][
                        "project_group_id"
                    ],
                }
            )
    ...
    def _load_exchange_rate_map(self, domain_id):
        if self.exchange_rate_map is None:
            ...
            exchange_rate_mgr: ExchangeRateManager = self.locator.get_manager(
                "ExchangeRateManager"
            )
            ...
            for exchange_rate_data in results:
                if exchange_rate_data.get("state", "ENABLED") == "ENABLED":
                    self.exchange_rate_map[exchange_rate_data["currency"]] = (
